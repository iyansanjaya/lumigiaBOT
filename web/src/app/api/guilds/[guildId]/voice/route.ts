...
import { validateVoiceSettingValue } from '@/lib/contracts';
...
export async function PATCH(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const validation = validateVoiceSettingValue(body.field, body.value);
    if ('error' in validation) {
      return NextResponse.json({ error: validation.error }, { status: 400 });
    }

    const success = updateVoiceSetting(guard.guildId, body.field, validation.value);
    if (!success) {
